...
class ZKBotController:
    ...
    # ==================== MOTOR CONTROL (DO PINS) ====================
    
    def motor_control(self, do_port: int, state: bool) -> Tuple[bool, str]:
        """
        Control a DO (Digital Output) pin for motor operation
        
        Args:
            do_port: DO port number (0-5 available, 6=air pump, 7=gripper)
            state: True for ON (S1), False for OFF (S0)
        
        Returns:
            Tuple[bool, str]: (success, response)
        """
        if not 0 <= do_port <= 7:
            return False, f"Invalid DO port: {do_port}"
        
        state_val = "S1" if state else "S0"
        cmd = f"0x550xAA G06 D{do_port} {state_val} P0 0xAA0x55"
        
        return self.send_command(cmd, timeout=2.0)
    
    def wash_motor_on(self) -> Tuple[bool, str]:
        """Turn ON wash station motor (DO1)"""
        print("🌊 WASH MOTOR ON (D01)")
        return self.motor_control(do_port=1, state=True)
    
    def wash_motor_off(self) -> Tuple[bool, str]:
        """Turn OFF wash station motor (DO1)"""
        print("🌊 WASH MOTOR OFF (D01)")
        return self.motor_control(do_port=1, state=False)
    
    def rinse_motor_on(self) -> Tuple[bool, str]:
        """Turn ON rinse station motor (D03)"""
        print("💧 RINSE MOTOR ON (D03)")
        return self.motor_control(do_port=3, state=True)
    
    def rinse_motor_off(self) -> Tuple[bool, str]:
        """Turn OFF rinse station motor (D03)"""
        print("💧 RINSE MOTOR OFF (D03)")
        return self.motor_control(do_port=3, state=False)
    
    def emergency_stop_motors(self) -> bool:
        """
        ...
        Returns:
            True if successful
        """
        print("\n🚑 EMERGENCY STOP: Shutting down all motors...")
        
        # Turn off wash motor
        success1, resp1 = self.wash_motor_off()
        if not success1:
            print(f"⚠️ Wash motor OFF failed: {resp1}")
        
        # Turn off rinse motor  
        success2, resp2 = self.rinse_motor_off()
        if not success2:
            print(f"⚠️ Rinse motor OFF failed: {resp2}")
